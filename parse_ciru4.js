...
let ciru4 = parseTextFile('ciru4_text.txt', 'cir');

fs.writeFileSync('parsed_ciru4.json', JSON.stringify(ciru4, null, 2));
console.log(`Parsed ${ciru4.length} new ciru cases from ciru4_text.txt`);

// Merge with questions.js directly
let oldQ = require('./questions.js');
let combined = oldQ.concat(ciru4);
...
console.log(`Successfully combined ${oldQ.length} old questions with ${ciru4.length} new cases. New total: ${combined.length}`);
