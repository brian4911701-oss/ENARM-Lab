...
(() => {
    ...
    const trackEvent = (name, params = {}) => {
        ...
        void window.FB.logAnalyticsEvent(name, params).catch((err) => {
            console.warn("[Analytics] Evento no registrado:", name, err);
        });
    };
    const ENARM_SPECIALTY_OPTIONS = [
        "Anatomía Patológica",
        "Anestesiología",
    ...
    const ENARM_TARGET_YEAR_OPTIONS = ["2026", "2027", "2028", "2029", "2030"];
    ...
    const FLASHCARD_SPECIALTIES = [
        {
            id: "mi",
            short: "MI",
            label: "Medicina Interna",
            tone: "green",
            description: "Cardio, neumo, nefro, endocrino, infecciosas y perlas integradoras."
        },
        {
            id: "ped",
            short: "PED",
            label: "Pediatria",
            tone: "blue",
            description: "Neonato, crecimiento, infecciones, urgencias pediatricas y vacunas."
        },
        {
            id: "gyo",
            short: "GYO",
            ...
            tone: "rose",
            description: "Control prenatal, sangrados, parto, puerperio, climaterio y oncogine."
        },
        {
            id: "cir",
            short: "CIR",
            label: "Cirugia General",
            tone: "orange",
            description: "Abdomen agudo, trauma, heridas, urgencias quirurgicas y posoperatorio."
        }
    ];
    const FLASHCARD_LIBRARY = {
        ...
        ped: [
            {
                topic: "Infecciones congenitas TORCH",
                cards: [
                    {
                        front: "Guia de patologia infecciosa congenita (TORCH): diagnostico y manejo",
                        back: "Repasa las claves de CMV, toxoplasmosis, rubeola, sifilis y varicela congenita: manifestaciones clasicas, diagnostico y manejo inicial.",
                        pearl: "Ubicacion: Pediatria > Infecciones congenitas TORCH.",
                        imageSrc: "Flashcards/unnamed%20(1).png",
                        imageAlt: "Infografia TORCH con CMV, toxoplasmosis, rubeola, sifilis y varicela congenita"
                    }
                ]
            },
            {
                topic: "Diarrea aguda en pediatria",
                cards: [
                    {
                        front: "Flashcard de estudio: diarrea aguda en pediatria",
                        back: "Resume definicion, agentes frecuentes, clasificacion OMS, metodo Dhaka, estudios utiles, rehidratacion y alertas de EHEC/SHU.",
                        pearl: "Ubicacion: Pediatria > Diarrea aguda en pediatria.",
                        imageSrc: "Flashcards/unnamed%20(2).png",
                        imageAlt: "Infografia de diarrea aguda en pediatria con clasificacion, diagnostico y plan de rehidratacion"
                    }
                ]
            }
        ],
    ...
    const normalizeThemeSelection = (theme) => {
        ...
        // Los temas retirados conservan la preferencia del usuario al migrar al claro lila.
    ...
    const updateReferralUI = () => {
        const coins = Number.isFinite(Number(State.coins)) ? Math.max(0, Math.floor(Number(State.coins))) : 0;
        const code = State.referralCode || "";
        const displayCode = code || (State.referralGenerationError ? "No se pudo generar" : "Generando...");
        localStorage.setItem("enarm_coins", String(coins));
        localStorage.setItem("enarm_referral_code", code);
        if ($("dash-coins")) $("dash-coins").textContent = coins.toLocaleString("es-MX");
        if ($("profile-coins")) $("profile-coins").value = coins.toLocaleString("es-MX");
        if ($("profile-referral-code")) $("profile-referral-code").value = code;
        if ($("referrals-code-display")) $("referrals-code-display").textContent = displayCode;
    ...
    const renderReferralsView = () => {
        const coins = Number.isFinite(Number(State.coins)) ? Math.max(0, Math.floor(Number(State.coins))) : 0;
        const code = State.referralCode || (State.referralGenerationError ? "No se pudo generar" : "Generando...");
        const minWithdraw = MIN_WITHDRAWAL_COINS;
        const remaining = Math.max(0, minWithdraw - coins);
        const progress = Math.max(0, Math.min(100, (coins / minWithdraw) * 100));
        if ($("referrals-code-display")) $("referrals-code-display").textContent = code;
        if ($("referrals-coin-balance")) $("referrals-coin-balance").textContent = coins.toLocaleString("es-MX");
        ...
        if ($("referrals-withdraw-status")) {
            $("referrals-withdraw-status").textContent = remaining > 0
                ? `Te faltan ${remaining.toLocaleString("es-MX")} monedas para retirar`
                : "Ya puedes solicitar tu retiro";
        }
        if ($("referrals-withdraw-copy")) {
            const friendsNeeded = Math.max(0, Math.ceil(remaining / REFERRAL_REWARD_COINS));
            $("referrals-withdraw-copy").textContent = remaining > 0
                ? `Invita a ${friendsNeeded} amigo${friendsNeeded !== 1 ? "s" : ""} mas para alcanzar el minimo de retiro.`
                : `Tu balance ya supera el minimo de ${MIN_WITHDRAWAL_COINS} monedas.`;
        }
        const amountInput = $("withdrawal-amount");
        if (amountInput) {
            amountInput.max = String(coins);
            amountInput.disabled = coins < MIN_WITHDRAWAL_COINS || State.withdrawalSubmitting;
            if (!amountInput.value && coins >= MIN_WITHDRAWAL_COINS) amountInput.value = String(MIN_WITHDRAWAL_COINS);
        }
        const submitBtn = $("btn-submit-withdrawal");
        if (submitBtn) {
            submitBtn.disabled = coins < MIN_WITHDRAWAL_COINS || State.withdrawalSubmitting;
            submitBtn.textContent = State.withdrawalSubmitting ? "Enviando..." : "Solicitar retiro";
        }
        const formStatus = $("withdrawal-form-status");
        if (formStatus) {
            formStatus.textContent = coins >= MIN_WITHDRAWAL_COINS
                ? `Puedes retirar entre ${MIN_WITHDRAWAL_COINS} y ${coins.toLocaleString("es-MX")} monedas.`
                : `Necesitas al menos ${MIN_WITHDRAWAL_COINS} monedas disponibles para solicitar un retiro.`;
        }
    };
    const buildReferralCodeCandidate = (uid, attempt = 0) => {
    ...
    const ensureReferralWallet = async (uid, displayName = "") => {
        ...
        if (walletSnap.exists()) {
            ...
            State.referralCode = normalizeReferralCode(data.referralCode || State.referralCode);
            updateReferralUI();
            return;
        }
        ...
        for (let attempt = 0; attempt < 8 && !created; attempt += 1) {
            const code = buildReferralCodeCandidate(uid, attempt);
            try {
                await window.FB.runTransaction(window.FB.db, async (tx) => {
                    ...
                    if (freshWalletSnap.exists()) {
                        const freshData = freshWalletSnap.data() || {};
                        State.coins = Number(freshData.coins) || 0;
                        State.referralCode = normalizeReferralCode(freshData.referralCode || "");
                        created = true;
                        return;
                    }
                    const codeRef = window.FB.doc(window.FB.db, "referral_codes", code);
                    ...
                    tx.set(codeRef, {
                        code,
                        ownerUid: uid,
                        ownerName: String(displayName || State.userName || "Aspirante").slice(0, 40),
                        createdAt: now
                    }, { merge: false });
                    tx.set(walletRef, {
                        uid,
                ...
                if (!String((err && err.message) || "").includes("referral_code_collision")) throw err;
            }
        }
        if (!created && !State.referralCode) {
            State.referralGenerationError = "No se pudo crear tu codigo automaticamente.";
        }
        updateReferralUI();
    };

    const handleReferralWalletError = (err) => {
        console.error("Error preparando wallet de referidos:", err);
        State.referralWalletLoaded = true;
        State.referralGenerationError = "No se pudo generar el codigo. Revisa reglas de Firestore o ejecuta el backfill.";
        updateReferralUI();
        if (State.view === "view-referrals") renderReferralsView();
    };

    const bindReferralWalletListener = (uid) => {
        if (!uid || !window.FB || !window.FB.db || !window.FB.doc || !window.FB.onSnapshot) return;
        if (State.referralWalletUnsub) {
            State.referralWalletUnsub();
            State.referralWalletUnsub = null;
        }
        const walletRef = window.FB.doc(window.FB.db, "user_wallets", uid);
        State.referralWalletUnsub = window.FB.onSnapshot(walletRef, (snap) => {
            State.referralWalletLoaded = true;
            if (!snap.exists()) {
                State.coins = 0;
                State.referralCode = "";
                updateReferralUI();
                void ensureReferralWallet(uid, State.userName).catch(handleReferralWalletError);
                return;
            }
        ...
        }, handleReferralWalletError);
    };

    const applyReferralRewardInTransaction = async (tx, uid, premiumCode, referralCode, now) => {
        const cleanReferralCode = normalizeReferralCode(referralCode);
        if (!cleanReferralCode) return false;
        if (cleanReferralCode === normalizeReferralCode(State.referralCode)) throw new Error("self_referral");

        const referralCodeRef = window.FB.doc(window.FB.db, "referral_codes", cleanReferralCode);
        const referralCodeSnap = await tx.get(referralCodeRef);
        if (!referralCodeSnap.exists()) throw new Error("invalid_referral");
        const referralData = referralCodeSnap.data() || {};
        const ownerUid = String(referralData.ownerUid || "");
        if (!ownerUid || ownerUid === uid) throw new Error("self_referral");

        const redemptionRef = window.FB.doc(window.FB.db, "referral_redemptions", uid);
        const redemptionSnap = await tx.get(redemptionRef);
        if (redemptionSnap.exists()) throw new Error("referral_used");

        const ownerWalletRef = window.FB.doc(window.FB.db, "user_wallets", ownerUid);
    ...
    function renderProfileView() {
        ...
        if (State.userTargetYear) metaParts.push(`ENARM ${State.userTargetYear}`);
        ...
        if ($("profile-referral-code")) $("profile-referral-code").value = State.referralCode || "";
        if ($("profile-coins")) $("profile-coins").value = (Number(State.coins) || 0).toLocaleString("es-MX");
    ...
    const syncPremiumUI = () => {
        ...
        const adminWithdrawalsPanel = $("admin-withdrawals-panel");
        if (adminWithdrawalsPanel) adminWithdrawalsPanel.style.display = isAdminUser() ? "block" : "none";
        const adminManualPaymentsPanel = $("admin-manual-payments-panel");
    ...
    const openRedeemModal = () => {
        ...
        if (reasonEl) reasonEl.textContent = "Esta herramienta es Premium. Elige un plan para continuar con la transferencia o canjea un código si ya tienes uno.";
    ...
    const getFlashcardTopicTotal = () => {
        return Object.values(FLASHCARD_LIBRARY || {}).reduce((sum, topics) => {
            return sum + (Array.isArray(topics) ? topics.length : 0);
        }, 0);
    };

    const getFlashcardCardTotal = () => {
        return Object.values(FLASHCARD_LIBRARY || {}).reduce((sum, topics) => {
            if (!Array.isArray(topics)) return sum;
            return sum + topics.reduce((topicSum, topic) => topicSum + ((topic.cards || []).length), 0);
        }, 0);
    };

    const getFlashcardSpecialtyMeta = (id) => {
        return FLASHCARD_SPECIALTIES.find(spec => spec.id === id) || FLASHCARD_SPECIALTIES[0];
    };

    const renderFlashcardsView = () => {
        const specialtyGrid = $("flashcards-specialty-grid");
        const topicList = $("flashcards-topic-list");
        const stackStage = $("flashcards-stack-stage");
        if (!specialtyGrid || !topicList || !stackStage) return;

        if (!FLASHCARD_SPECIALTIES.some(spec => spec.id === State.flashcardsSpecialty)) {
            State.flashcardsSpecialty = FLASHCARD_SPECIALTIES[0]?.id || "mi";
        }

        const activeSpec = getFlashcardSpecialtyMeta(State.flashcardsSpecialty);
        const activeTopics = FLASHCARD_LIBRARY[activeSpec.id] || [];
        const activeCards = activeTopics.flatMap(topic => {
            return (topic.cards || []).map(card => ({ ...card, topic: topic.topic || "Tema pendiente" }));
        });

        const specialtyCountEl = $("flashcards-specialty-count");
        const topicCountEl = $("flashcards-topic-count");
        const cardCountEl = $("flashcards-card-count");
        const activeKicker = $("flashcards-active-kicker");
        const activeTitle = $("flashcards-active-title");
        if (specialtyCountEl) specialtyCountEl.textContent = String(FLASHCARD_SPECIALTIES.length);
        if (topicCountEl) topicCountEl.textContent = String(getFlashcardTopicTotal());
        if (cardCountEl) cardCountEl.textContent = String(getFlashcardCardTotal());
        if (activeKicker) activeKicker.textContent = `${activeTopics.length} temas / ${activeCards.length} tarjetas`;
        if (activeTitle) activeTitle.textContent = activeSpec.label;

        specialtyGrid.innerHTML = FLASHCARD_SPECIALTIES.map(spec => {
            const topics = FLASHCARD_LIBRARY[spec.id] || [];
            const cardCount = topics.reduce((sum, topic) => sum + ((topic.cards || []).length), 0);
            const active = spec.id === activeSpec.id;
            return `
                <button class="flashcards-specialty-tile ${active ? "active" : ""} tone-${escapeHtml(spec.tone)}" type="button" data-flash-spec="${escapeHtml(spec.id)}">
                    <span class="flashcards-specialty-code">${escapeHtml(spec.short)}</span>
                    <strong>${escapeHtml(spec.label)}</strong>
                    <p>${escapeHtml(spec.description)}</p>
                    <small>${topics.length} temas / ${cardCount} tarjetas</small>
                </button>
            `;
        }).join("");

        if (activeTopics.length) {
            topicList.innerHTML = activeTopics.map(topic => {
                const cards = topic.cards || [];
                return `
                    <article class="flashcards-topic-chip">
                        <span>${escapeHtml(topic.topic || "Tema pendiente")}</span>
            ...
            topicList.innerHTML = `
                <div class="flashcards-topic-empty">
                    <span>Sin temas cargados</span>
                    <p>Esta especialidad ya tiene su lugar reservado. Cuando lleguen tarjetas, las vamos agrupando por tema clinico.</p>
                </div>
            `;
        }

        if (activeCards.length) {
            stackStage.innerHTML = activeCards.slice(0, 4).map((card, idx) => `
                <article class="flashcard-study-card" style="--card-i:${idx};">
                    <span>${escapeHtml(card.topic)}</span>
                    <h3>${escapeHtml(card.front || "Pregunta pendiente")}</h3>
                    ${card.imageSrc ? `
                        <button class="flashcard-study-media" type="button"
                            data-flashcard-image="${escapeHtml(card.imageSrc)}"
                            data-flashcard-title="${escapeHtml(card.front || "Flashcard")}"
                            data-flashcard-topic="${escapeHtml(card.topic || "Tema")}"
                            data-flashcard-copy="${escapeHtml(card.back || "")}"
                            data-flashcard-alt="${escapeHtml(card.imageAlt || card.front || "Flashcard")}">
                            <img class="flashcard-study-image" src="${escapeHtml(card.imageSrc)}" alt="${escapeHtml(card.imageAlt || card.front || "Flashcard")}">
                            <span class="flashcard-study-zoom">Tocar para ampliar</span>
                        </button>
                    ` : ""}
                    <p>${escapeHtml(card.back || "Respuesta pendiente")}</p>
                    ${card.pearl ? `<small>${escapeHtml(card.pearl)}</small>` : ""}
                </article>
            `).join("");
        } else {
            stackStage.innerHTML = `
                <div class="flashcards-empty-card">
                    <span>Lista para cargar</span>
                    <h3>Sin tarjetas todavia</h3>
                    <p>Pasame flashcards poco a poco y las voy a ir acomodando aqui por especialidad y tema, con una estructura limpia para repasar sin friccion.</p>
                </div>
            `;
        }

        $$(".flashcards-specialty-tile").forEach(btn => {
            btn.addEventListener("click", () => {
                State.flashcardsSpecialty = btn.dataset.flashSpec || "mi";
                renderFlashcardsView();
            });
        });

        $$(".flashcard-study-media").forEach(btn => {
            btn.addEventListener("click", () => {
                openFlashcardImageModal({
                    src: btn.dataset.flashcardImage || "",
                    title: btn.dataset.flashcardTitle || "Flashcard",
                    topic: btn.dataset.flashcardTopic || "Tema",
                    copy: btn.dataset.flashcardCopy || "",
                    alt: btn.dataset.flashcardAlt || "Flashcard"
                });
            });
        });
    };

    const openFlashcardImageModal = ({ src, title, topic, copy, alt }) => {
        const modal = $("flashcard-image-modal");
        const image = $("flashcard-modal-image");
        const titleEl = $("flashcard-modal-title");
        const topicEl = $("flashcard-modal-topic");
        const copyEl = $("flashcard-modal-copy");
        if (!modal || !image || !titleEl || !topicEl || !copyEl || !src) return;
        image.src = src;
        image.alt = alt || title || "Flashcard";
        titleEl.textContent = title || "Flashcard";
        topicEl.textContent = topic || "Tema";
        copyEl.textContent = copy || "";
        document.body.classList.add("flashcard-modal-open");
        modal.style.display = "flex";
    };

    const closeFlashcardImageModal = () => {
        const modal = $("flashcard-image-modal");
        const image = $("flashcard-modal-image");
    ...
    const syncFlashcardsAccessUI = () => {
        const allowed = isAdminUser();
        const homeCard = $("flashcards-home-card");
        const homeBtn = $("btn-flashcards-home");
        const moreItem = $("mas-flashcards-item");
        const moreBadge = $("mas-flashcards-badge");

        if (homeCard) {
            homeCard.classList.toggle("is-locked", !allowed);
            homeCard.classList.toggle("is-admin-ready", allowed);
        }
        if (homeBtn) {
            homeBtn.disabled = !allowed;
            homeBtn.textContent = allowed ? "Abrir flashcards" : "Pr\u00f3ximamente";
            homeBtn.title = allowed ? "Abrir laboratorio de flashcards" : "Disponible pr\u00f3ximamente";
            homeBtn.setAttribute("aria-disabled", allowed ? "false" : "true");
        }
        if (moreItem) {
            moreItem.classList.toggle("is-locked", !allowed);
            moreItem.classList.toggle("is-admin-ready", allowed);
            moreItem.setAttribute("aria-disabled", allowed ? "false" : "true");
            moreItem.title = allowed ? "Abrir laboratorio de flashcards" : "Disponible pr\u00f3ximamente";
            moreItem.tabIndex = allowed ? 0 : -1;
        }
        if (moreBadge) {
            moreBadge.textContent = allowed ? "Admin" : "Pr\u00f3ximamente";
        }
        if (!allowed && State.view === "view-flashcards") {
            showView("view-dashboard");
        }
    };

    const openFlashcardsGate = () => {
    ...
    const initFlashcardsEntryPoints = () => {
        const bindEntry = (el) => {
            if (!el || el.dataset.flashcardsBound === "1") return;
            el.dataset.flashcardsBound = "1";
        ...
        bindEntry($("btn-flashcards-home"));
        bindEntry($("mas-flashcards-item"));
        syncFlashcardsAccessUI();
    };
    ...
    const normalizeWithdrawalAmount = (value) => {
        const amount = Math.floor(Number(value));
        return Number.isFinite(amount) ? amount : 0;
    };

    const renderAdminWithdrawalRequests = () => {
        ...
        const rows = Array.isArray(State.withdrawalRequests) ? State.withdrawalRequests : [];
        if (rows.length === 0) {
            list.innerHTML = `<div class="list-item empty-history"><p style="color:var(--text-muted); padding: 20px;">No hay solicitudes de retiro pendientes o recientes.</p></div>`;
            return;
        }

        list.innerHTML = rows.map((item) => {
            const amount = normalizeWithdrawalAmount(item.amount);
            const createdAt = getWithdrawalDate(item.createdAt);
            const paidAt = getWithdrawalDate(item.paidAt);
            const createdLabel = createdAt ? formatDateTime(createdAt) : "Sin fecha";
        ...
        list.querySelectorAll("[data-confirm-withdrawal]").forEach((btn) => {
            btn.addEventListener("click", () => confirmWithdrawalPaid(btn.dataset.confirmWithdrawal));
        });
    };

    const initWithdrawalAdminInbox = () => {
        if (!isAdminUser()) {
        ...
        if (State.withdrawalAdminUnsub) return;
        if (!window.FB || !window.FB.db || !window.FB.auth || !window.FB.auth.currentUser) return;

        const ref = window.FB.collection(window.FB.db, WITHDRAWAL_REQUESTS_COLLECTION);
        const q = window.FB.query(ref, window.FB.orderBy("createdAt", "desc"), window.FB.limit(100));
        State.withdrawalAdminUnsub = window.FB.onSnapshot(q, (snap) => {
            const rows = [];
            snap.forEach((docSnap) => {
                rows.push({ id: docSnap.id, ...(docSnap.data() || {}) });
            });
            State.withdrawalRequests = rows;
            renderAdminWithdrawalRequests();
        }, (err) => {
            console.error("Error cargando solicitudes de retiro:", err);
            const msg = String((err && (err.code || err.message)) || "").toLowerCase();
            if (msg.includes("permission-denied") || msg.includes("missing or insufficient permissions")) {
                showNotification("No tienes permisos para ver solicitudes de retiro en Firebase.", "error");
            }
        });
    };
    ...
    const renderManualPaymentRequests = () => {
        ...
        list.querySelectorAll("[data-review-payment]").forEach((btn) => {
            btn.addEventListener("click", () => reviewManualPayment(
                btn.dataset.paymentRequestId,
                btn.dataset.reviewPayment
            ));
        });
    };

    const initManualPaymentAdminInbox = () => {
        if (!isAdminUser()) {
            if (State.manualPaymentAdminUnsub) {
                State.manualPaymentAdminUnsub();
                State.manualPaymentAdminUnsub = null;
            }
            State.manualPaymentRequests = [];
            renderManualPaymentRequests();
            return;
        }
        if (State.manualPaymentAdminUnsub) return;
        if (!window.FB || !window.FB.db || !window.FB.auth || !window.FB.auth.currentUser) return;

        const ref = window.FB.collection(window.FB.db, MANUAL_PAYMENT_REQUESTS_COLLECTION);
        const q = window.FB.query(ref, window.FB.orderBy("createdAt", "desc"), window.FB.limit(100));
        State.manualPaymentAdminUnsub = window.FB.onSnapshot(q, (snap) => {
            const rows = [];
            snap.forEach((docSnap) => rows.push({ id: docSnap.id, ...(docSnap.data() || {}) }));
            State.manualPaymentRequests = rows;
            renderManualPaymentRequests();
            renderAdminUsers();
        }, (err) => {
            console.error("Error cargando transferencias Premium:", err);
            showNotification("No se pudieron cargar los avisos de transferencia.", "error");
        });
    };
    ...
    const getLatestPaymentForUser = (uid) => {
        return (State.manualPaymentRequests || [])
            ...
            .sort((a, b) => {
                const aDate = getWithdrawalDate(a.updatedAt || a.notifiedAt || a.createdAt)?.getTime() || 0;
                const bDate = getWithdrawalDate(b.updatedAt || b.notifiedAt || b.createdAt)?.getTime() || 0;
                return bDate - aDate;
            })[0] || null;
    };
    ...
    const renderAdminUsers = () => {
        ...
        list.querySelectorAll("[data-user-premium-toggle]").forEach((toggle) => {
            toggle.addEventListener("change", () => setAdminUserPremium(toggle.dataset.userPremiumToggle, toggle.checked));
        });
    };
    ...
    const initAdminUsersInbox = () => {
        const stop = () => {
            if (State.adminUsersUnsub) State.adminUsersUnsub();
            if (State.adminDirectoryUnsub) State.adminDirectoryUnsub();
            if (State.adminEntitlementsUnsub) State.adminEntitlementsUnsub();
            State.adminUsersUnsub = null;
    ...
    const bindMyManualPaymentRequests = (uid) => {
        if (State.myManualPaymentUnsub) {
            State.myManualPaymentUnsub();
            State.myManualPaymentUnsub = null;
        }
        State.myManualPaymentRequests = [];
        syncPaymentPendingBanner();
        if (!uid || !window.FB || !window.FB.db || !window.FB.onSnapshot || !window.FB.query || !window.FB.where) return;
        const ref = window.FB.collection(window.FB.db, MANUAL_PAYMENT_REQUESTS_COLLECTION);
        const q = window.FB.query(ref, window.FB.where("uid", "==", uid));
        State.myManualPaymentUnsub = window.FB.onSnapshot(q, (snap) => {
            const rows = [];
            snap.forEach((docSnap) => rows.push({ id: docSnap.id, ...(docSnap.data() || {}) }));
            State.myManualPaymentRequests = rows;
            syncPaymentPendingBanner();
        }, (err) => {
            console.error("Error cargando el estado del pago:", err);
        });
    };

    const reviewManualPayment = async (requestId, decision) => {
        if (!requestId || !isAdminUser() || !["approved", "rejected"].includes(decision)) return;
        const actionLabel = decision === "approved" ? "aprobar y activar Premium" : "rechazar";
        if (!window.confirm(`¿Confirmas que deseas ${actionLabel} para esta solicitud?`)) return;
        if (!window.FB || !window.FB.db || !window.FB.doc || !window.FB.runTransaction) {
            showNotification("Firebase todavía no está disponible.", "error");
            return;
        }
        try {
            await window.FB.runTransaction(window.FB.db, async (tx) => {
                const paymentRef = window.FB.doc(window.FB.db, MANUAL_PAYMENT_REQUESTS_COLLECTION, requestId);
                ...
                const plan = TRANSFER_PLANS[payment.planId];
                if (!plan || !payment.uid) throw new Error("payment_invalid");
                if (decision === "approved" && new Date(plan.expiresAt).getTime() <= Date.now()) {
                ...
                if (decision === "approved") {
                    entitlementRef = window.FB.doc(window.FB.db, "entitlements", payment.uid);
                    const entitlementSnap = await tx.get(entitlementRef);
                    currentEntitlement = entitlementSnap.exists() ? (entitlementSnap.data() || {}) : {};
                }
                ...
                tx.update(paymentRef, {
                    status: decision,
                    reviewedAt: now,
                    reviewedByUid: State.currentUid,
                    updatedAt: now
                });
                if (decision === "approved") {
                    const planExpiry = new Date(plan.expiresAt);
                    const currentExpiry = currentEntitlement.expiresAt && typeof currentEntitlement.expiresAt.toDate === "function"
                        ? currentEntitlement.expiresAt.toDate().getTime()
                        : new Date(currentEntitlement.expiresAt || 0).getTime();
                    if (currentEntitlement.status !== "active" || currentExpiry < planExpiry.getTime()) {
                        tx.set(entitlementRef, {
                            status: "active",
                            source: "manual_transfer",
                            planId: plan.id,
                            expiresAt: planExpiry,
                            transferReference: String(payment.transferReference || ""),
            ...
            showNotification(
                decision === "approved" ? "Pago aprobado y Premium activado." : "Aviso de transferencia rechazado.",
    ...
    const submitWithdrawalRequest = async () => {
        ...
        const amount = normalizeWithdrawalAmount($("withdrawal-amount")?.value);
        const fullName = $("withdrawal-full-name")?.value?.trim() || "";
        const accountHolder = $("withdrawal-account-holder")?.value?.trim() || "";
        const bankName = $("withdrawal-bank-name")?.value?.trim() || "";
        const clabe = ($("withdrawal-clabe")?.value || "").replace(/\D/g, "");
        const phone = $("withdrawal-phone")?.value?.trim() || "";
        const confirmed = $("withdrawal-confirm-data")?.checked;

        if (coins < MIN_WITHDRAWAL_COINS) {
            showNotification(`Necesitas al menos ${MIN_WITHDRAWAL_COINS} monedas para retirar.`, "warning");
            return;
        }
        if (amount < MIN_WITHDRAWAL_COINS) {
            showNotification(`El retiro minimo es de ${MIN_WITHDRAWAL_COINS} monedas.`, "warning");
            return;
        }
        if (amount > coins) {
            showNotification("No puedes solicitar mas monedas de las que tienes disponibles.", "warning");
            return;
        }
        if (!fullName || !accountHolder || !bankName) {
            showNotification("Completa tu nombre, titular de cuenta y banco.", "warning");
            return;
        }
        if (!/^\d{18}$/.test(clabe)) {
            showNotification("La CLABE debe tener exactamente 18 digitos.", "warning");
            return;
        }
        if (!confirmed) {
            showNotification("Confirma que tus datos bancarios son correctos.", "warning");
            return;
        }
        ...
        try {
            ...
            ["withdrawal-amount", "withdrawal-full-name", "withdrawal-account-holder", "withdrawal-bank-name", "withdrawal-clabe", "withdrawal-phone"].forEach((id) => {
                const el = $(id);
                if (el) el.value = "";
            });
            const confirmEl = $("withdrawal-confirm-data");
            if (confirmEl) confirmEl.checked = false;
            showNotification("Solicitud de retiro enviada. Revisaremos tus datos y el pago puede reflejarse en 1 a 5 dias habiles.", "success");
        } catch (err) {
            console.error("Error solicitando retiro:", err);
            const msg = String((err && (err.code || err.message)) || "").toLowerCase();
            if (msg.includes("permission-denied") || msg.includes("missing or insufficient permissions")) {
                showNotification("Firebase rechazo la solicitud. Hay que revisar las reglas de retiro.", "error");
            } else {
                showNotification("No se pudo enviar la solicitud de retiro.", "error");
            }
        } finally {
    ...
    const confirmWithdrawalPaid = async (requestId) => {
        if (!requestId || !isAdminUser()) {
        ...
        if (!window.confirm("Confirma solo si la transferencia bancaria ya fue realizada. Al aceptar se descontaran las monedas del usuario.")) {
            return;
        }
        try {
            await window.FB.runTransaction(window.FB.db, async (tx) => {
                const requestRef = window.FB.doc(window.FB.db, WITHDRAWAL_REQUESTS_COLLECTION, requestId);
                const requestSnap = await tx.get(requestRef);
                if (!requestSnap.exists()) throw new Error("withdrawal_not_found");
                const request = requestSnap.data() || {};
                if (String(request.status || "pending") !== "pending") throw new Error("withdrawal_not_pending");
                const uid = String(request.uid || "");
                const amount = normalizeWithdrawalAmount(request.amount);
                if (!uid || amount < MIN_WITHDRAWAL_COINS) throw new Error("withdrawal_invalid");

                const walletRef = window.FB.doc(window.FB.db, "user_wallets", uid);
                const walletSnap = await tx.get(walletRef);
                if (!walletSnap.exists()) throw new Error("wallet_missing");
                const wallet = walletSnap.data() || {};
                const currentCoins = Math.max(0, Math.floor(Number(wallet.coins) || 0));
                if (currentCoins < amount) throw new Error("insufficient_wallet_balance");
                const now = new Date();
                tx.update(walletRef, {
                ...
                tx.update(requestRef, {
                    status: "paid",
                    paidAt: now,
                    paidByUid: window.FB.auth.currentUser.uid,
                    updatedAt: now,
                    walletBalanceBeforePayment: currentCoins,
                    walletBalanceAfterPayment: currentCoins - amount
                });
            });
            showNotification("Retiro confirmado y monedas descontadas.", "success");
        } catch (err) {
            console.error("Error confirmando retiro:", err);
            const msg = String((err && err.message) || "").toLowerCase();
            if (msg.includes("insufficient_wallet_balance")) {
                showNotification("El usuario ya no tiene saldo suficiente para descontar ese retiro.", "error");
            } else if (msg.includes("withdrawal_not_pending")) {
                showNotification("Esta solicitud ya fue procesada.", "info");
            } else {
                showNotification("No se pudo confirmar el retiro.", "error");
            }
        }
    };
    ...
    const showView = (viewId) => {
        ...
        if (viewId === "view-flashcards" && !isAdminUser()) {
            showNotification("Flashcards estar\u00e1 disponible pr\u00f3ximamente.", "info");
            return;
        }
        ...
        if (viewId === "view-referrals") renderReferralsView();
    ...
    const loadGlobalStats = () => {
        ...
        State.referralCode = normalizeReferralCode(localStorage.getItem("enarm_referral_code") || "");
        updateReferralUI();
    ...
    const applyTheme = (theme) => {
        ...
        document.body.classList.remove("light-mode", "theme-lilac-light", "theme-forest", "theme-ocean", "theme-sunset", "theme-navy-gold", "theme-black-teal", "theme-aurora", "theme-ember", "theme-black-white", "theme-premium", "theme-premium-pink", "theme-clinical-dark", "theme-clinical-light");
        ...
        if (normalizedTheme === "system") {
            ...
            document.body.classList.add("light-mode");
        } else if (normalizedTheme === "lilac-light") {
            document.body.classList.add("theme-lilac-light");
    ...
    const isPremiumResolutionPending = () => {
        if (!window.FB || !window.FB.auth) return !State.authStateResolved;
        if (!State.authStateResolved) return true;
        const currentUser = window.FB.auth.currentUser;
        if (!currentUser) return false;
        return !State.entitlementLoaded || !State.globalPremiumLoaded;
    };
    ...
    let lastStudyDashboardRenderKey = "";
    const renderStudyDashboard = () => {
        if (State.view !== "view-dashboard" && State.view !== "view-estudio-plus") return;
        ensureStudySystemsState();
        rebuildTopicMastery();
        ensureDailyPlanFresh();
        ...
        const todayKey = formatDateKey();
        const studyDashboardRenderKey = [
            State.view,
            plan.summary || "",
            plan.focusTopic || "",
            plan.pressure || "",
            (plan.tasks || []).map(task => `${task.id}:${task.title}:${(plan.completedTaskIds || []).includes(task.id) ? 1 : 0}`).join("|"),
            (State.reviewQueue || []).length,
            (State.reviewQueue || []).slice(0, 6).map(item => `${item.id}:${item.dueDate || ""}:${item.completedAt ? 1 : 0}:${item.topic || ""}`).join("|"),
            (State.caseNotebook || []).length,
            (State.caseNotebook || []).slice(0, 3).map(entry => `${entry.id}:${entry.title || ""}:${entry.topic || ""}:${String(entry.note || "").length}`).join("|"),
            `${masteryCounts.total}-${masteryCounts.dominado}-${masteryCounts.en_progreso}-${masteryCounts.en_riesgo}`,
            JSON.stringify(State.studyCalendar?.[todayKey] || {})
        ].join("::");
        if (studyDashboardRenderKey === lastStudyDashboardRenderKey) return;
        lastStudyDashboardRenderKey = studyDashboardRenderKey;
    ...
    let lastResultsPostmortemRenderKey = "";
    const renderResultsPostmortem = () => {
        if (State.view !== "view-results" && State.view !== "view-estudio-plus") return;
        ...
        const postmortemRenderKey = `${State.view}::${JSON.stringify(postmortem || null)}`;
        if (postmortemRenderKey === lastResultsPostmortemRenderKey) return;
        lastResultsPostmortemRenderKey = postmortemRenderKey;
        if (!postmortem) {
    ...
    const renderRefuerzosView = () => {
        if (State.view !== "view-dashboard" && State.view !== "view-refuerzos") return;
    ...
    const updateDashboardStats = (force = false) => {
        ...
        if ($("dash-coins")) $("dash-coins").textContent = (Number(State.coins) || 0).toLocaleString("es-MX");
    ...
    const formatSessionKindLabel = (kind) => {
        if (kind === "estudio") return "Estudio";
        if (kind === "repaso") return "Repaso";
        return "Simulacro";
    };

    const getStatsPdfReportData = () => {
        const answered = Number(State.globalStats?.respondidas) || 0;
        const correct = Number(State.globalStats?.aciertos) || 0;
        const omitted = Number(State.globalStats?.totalBlank) || 0;
        const errors = Math.max(answered - correct, 0);
        const precision = getGlobalAccuracy();
        const streak = getEffectiveStreak();
        const probability = getProbability(streak);
        const averageSeconds = (() => {
            ...
            recentSessions.forEach((session) => {
                const questions = Number(session?.preguntas) || 0;
                const elapsedSec = Number(session?.elapsedSec) || 0;
                if (questions > 0 && elapsedSec > 0) {
                    totalSec += elapsedSec;
                    totalQ += questions;
                }
            });
            return totalQ > 0 ? Math.round(totalSec / totalQ) : 0;
        })();

        const specialtyRows = ["mi", "ped", "gyo", "cir"].map((key) => {
            const stats = State.globalStats?.bySpecialty?.[key] || {};
            const total = Number(stats.total) || 0;
            const specialtyCorrect = Number(stats.correct) || 0;
            const specialtyPrecision = total > 0 ? Number(((specialtyCorrect / total) * 100).toFixed(1)) : 0;
            return {
                key,
                name: stats.name || TRONCAL_LABELS[key] || key,
                total,
                correct: specialtyCorrect,
                precision: specialtyPrecision
            };
        });
        ...
        const weakTopics = getTopicPerformanceEntries()
            ...
            .map((entry) => {
                ...
                const correctTopic = Number(entry.correct) || 0;
                const wrong = Math.max(total - correctTopic, 0);
                return {
                    topic: entry.topic,
                    specialty: entry.specialtyLabel || (TRONCAL_LABELS[entry.specialty] || "General"),
        ...
        const recentSessions = [...(State.history || [])]
            ...
            .map((session, index) => {
                ...
                const elapsedSec = Number(session?.elapsedSec) || 0;
                return {
                    id: `${session?.timestamp || "session"}-${index}`,
                    date: session?.timestamp ? formatDateTime(new Date(session.timestamp)) : "Sin fecha",
                    kind: formatSessionKindLabel(session?.sessionKind),
                    precision: Number(session?.pct) || 0,
                    questions,
                    blanks: Number(session?.blank) || 0,
                    pace: questions > 0 && elapsedSec > 0 ? Math.round(elapsedSec / questions) : 0
                };
            });

        const strongestSpecialty = [...specialtyRows].sort((a, b) => b.precision - a.precision || b.total - a.total)[0];
        const mostPracticedSpecialty = [...specialtyRows].sort((a, b) => b.total - a.total || b.precision - a.precision)[0];
        const weakestTopic = weakTopics[0] || null;
        const premiumLabel = isAdminUser()
            ? (State.adminPreviewMode === "demo" ? "Admin Demo" : "Admin Premium")
            : (isGlobalPremiumPromoActive() ? "Premium global" : (isPremiumActive() ? "Premium" : "Demo"));
        const reportDate = new Date();
        const reportSlug = (State.userName || "aspirante")
            .toLowerCase()
            .normalize("NFD")
            .replace(/[\u0300-\u036f]/g, "")
            .replace(/[^a-z0-9]+/g, "-")
        ...
        return {
            ...
            userName: State.userName || "Aspirante",
            specialtyGoal: State.userSpecialty || "Objetivo no definido",
            university: State.userUniversity || "Universidad no registrada",
            premiumLabel,
            rank: getUserRankLabel(precision),
            streak,
            probability: Number(probability || 0).toFixed(1),
            precision: precision.toFixed(1),
            answered,
            correct,
            errors,
            omitted,
            sessions: Number(State.globalStats?.sesiones) || 0,
            pomodoros: Number(State.globalStats?.pomodoros) || 0,
            averageSeconds,
            strongestSpecialty,
            mostPracticedSpecialty,
            weakestTopic,
            specialtyRows,
            weakTopics,
            recentSessions
    ...
    const buildStatsPdfHtml = (report) => {
        const style = getComputedStyle(document.body);
        const theme = {
            bgMain: style.getPropertyValue("--bg-main").trim() || "#07111f",
            bgCard: style.getPropertyValue("--bg-card").trim() || "#0f172a",
            ...
            border: style.getPropertyValue("--border").trim() || "rgba(148, 163, 184, 0.24)",
            accentGreen: style.getPropertyValue("--accent-green").trim() || "#05C07F",
            accentBlue: style.getPropertyValue("--accent-blue").trim() || "#3b82f6",
            accentOrange: style.getPropertyValue("--accent-orange").trim() || "#f37a20",
            accentRed: style.getPropertyValue("--accent-red").trim() || "#ef4444",
            fontUi: style.getPropertyValue("--font-ui").trim() || "Inter, Arial, sans-serif",
            fontDisplay: style.getPropertyValue("--font-display").trim() || style.getPropertyValue("--font-ui").trim() || "Inter, Arial, sans-serif"
        };

        const summaryCards = [
            { label: "Precision global", value: `${report.precision}%`, note: `${report.correct} aciertos de ${report.answered}` },
            { label: "Preguntas trabajadas", value: report.answered.toLocaleString("es-MX"), note: `${report.errors} errores y ${report.omitted} omitidas` },
            { label: "Ritmo promedio", value: `${report.averageSeconds}s`, note: "Calculado sobre tus 10 sesiones recientes" },
            { label: "Constancia", value: `${report.streak} dias`, note: `${report.sessions} sesiones cerradas y ${report.pomodoros} pomodoros` }
        ];

        const specialtyRows = report.specialtyRows.map((row) => `
            <tr>
                <td>${escapeHtml(row.name)}</td>
                <td>${row.total.toLocaleString("es-MX")}</td>
                <td>${row.correct.toLocaleString("es-MX")}</td>
                <td>${row.precision.toFixed(1)}%</td>
            </tr>
        `).join("");

        const weakTopicsMarkup = report.weakTopics.length
            ? report.weakTopics.map((topic, index) => `
                <article class="topic-card">
                    <div class="topic-index">0${index + 1}</div>
                    <div class="topic-copy">
                        <h4>${escapeHtml(topic.topic)}</h4>
                        <p>${escapeHtml(topic.specialty)} · ${topic.total} reactivos · ${topic.wrong} fallos</p>
                    </div>
                    <div class="topic-score">${topic.precision.toFixed(1)}%</div>
                </article>
            `).join("")
            : `<div class="empty-state">Todavia no hay suficientes datos por tema para priorizar debilidades.</div>`;

        const sessionsMarkup = report.recentSessions.length
            ? report.recentSessions.map((session) => `
                <tr>
                    <td>${escapeHtml(session.date)}</td>
                    <td>${escapeHtml(session.kind)}</td>
                    <td>${session.questions}</td>
                    <td>${session.precision.toFixed(1)}%</td>
                    <td>${session.blanks}</td>
                    <td>${session.pace ? `${session.pace}s` : "Sin dato"}</td>
                </tr>
            `).join("")
            : `<tr><td colspan="6" class="empty-row">Aun no hay sesiones guardadas.</td></tr>`;
...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        html, body { margin: 0; padding: 0; background: #e8edf5; color: #0f172a; font-family: var(--font-ui); }
        body { padding: 24px; }
        .sheet {
            width: 100%;
            max-width: 960px;
        ...
        .hero {
            ...
            color: white;
            background:
                radial-gradient(circle at top left, rgba(5, 192, 127, 0.32), transparent 28%),
                radial-gradient(circle at 85% 20%, rgba(243, 122, 32, 0.24), transparent 24%),
                linear-gradient(135deg, var(--bg-main) 0%, color-mix(in srgb, var(--bg-card) 92%, black) 100%);
        }
        .hero-top {
            display: flex;
        ...
        .insight-card::before {
            ...
            position: absolute;
            inset: 0;
            background: linear-gradient(135deg, rgba(59,130,246,0.08), transparent 58%);
            pointer-events: none;
        }
        .insight-card h3 {
            margin: 0 0 8px;
        ...
        .toolbar .primary {
            background: linear-gradient(135deg, var(--accent-green), color-mix(in srgb, var(--accent-green) 72%, black));
            color: white;
        }
        .toolbar .secondary {
            background: white;
...
<body>
            ...
            <div class="hero-top">
                <div>
                    ...
                    <div class="hero-pills">
                        <span class="pill">Objetivo: ${escapeHtml(report.specialtyGoal)}</span>
                        <span class="pill">${escapeHtml(report.rank)}</span>
                        <span class="pill">Acceso ${escapeHtml(report.premiumLabel)}</span>
                    </div>
                </div>
                <div class="hero-meta">
                    <div><strong>${escapeHtml(report.generatedShort)}</strong> Fecha de generacion</div>
                    <div><strong>${escapeHtml(report.university)}</strong> Universidad</div>
                    <div><strong>${report.probability}%</strong> Probabilidad por constancia</div>
                </div>
            </div>
        </section>
        <section class="content">
            ...
            <section class="insights">
                <article class="insight-card">
                    <h3>Especialidad mas fuerte</h3>
                    <strong>${escapeHtml(report.strongestSpecialty?.name || "Sin datos")}</strong>
                    <p>${report.strongestSpecialty ? `${report.strongestSpecialty.precision.toFixed(1)}% de precision con ${report.strongestSpecialty.total} reactivos.` : "Todavia no hay suficientes preguntas para estimarlo."}</p>
                </article>
                <article class="insight-card">
                    <h3>Mayor volumen de practica</h3>
                    <strong>${escapeHtml(report.mostPracticedSpecialty?.name || "Sin datos")}</strong>
                    <p>${report.mostPracticedSpecialty ? `${report.mostPracticedSpecialty.total} reactivos acumulados en esta area.` : "Todavia no hay volumen para medir cobertura."}</p>
                </article>
                <article class="insight-card">
                    <h3>Foco inmediato</h3>
                    <strong>${escapeHtml(report.weakestTopic?.topic || "Sin prioridad aun")}</strong>
                    <p>${report.weakestTopic ? `${report.weakestTopic.precision.toFixed(1)}% en ${escapeHtml(report.weakestTopic.specialty)}.` : "Tu panel aun no tiene suficiente historial por tema."}</p>
                </article>
            </section>
            ...
            <section class="block">
                <div class="section-title">
                    <h2>Temas mas fragiles</h2>
                    <span>Top accionable</span>
                </div>
                <div class="topics-grid">${weakTopicsMarkup}</div>
            </section>
    ...
    const openStatsPdfReport = () => {
        const report = getStatsPdfReportData();
        const existingFrame = $("stats-pdf-export-frame");
        if (existingFrame && existingFrame.parentNode) existingFrame.parentNode.removeChild(existingFrame);

        const frame = document.createElement("iframe");
        frame.id = "stats-pdf-export-frame";
        frame.title = "Exportacion PDF de estadisticas";
        ...
        frame.onload = () => {
            ...
            if (!printWindow) {
                cleanup();
                showNotification("No se pudo preparar el PDF en este navegador.", "error", 5000);
                return;
            }

            const afterPrintHandler = () => {
                cleanup();
                try { printWindow.removeEventListener("afterprint", afterPrintHandler); } catch (_) {}
            };

            try { printWindow.addEventListener("afterprint", afterPrintHandler); } catch (_) {}

            setTimeout(() => {
                try { printWindow.focus(); } catch (_) {}
                try {
                    printWindow.print();
                    showNotification("Reporte preparado. Elige 'Guardar como PDF' en la ventana de impresion.", "success", 5000);
                } catch (_) {
                    showNotification("No se pudo abrir la impresion automaticamente. Intenta de nuevo.", "warning", 5000);
                    cleanup();
                }
            }, 250);
        };

        frame.srcdoc = buildStatsPdfHtml(report);
    ...
    let lastPomodoroStaticRenderKey = "";
    ...
    const getPomodoroStaticRenderKey = (weeklySeries, recentLog, selectedSpecialtyLabels, todayPomodoros, streak) => {
        const settings = State.pomodoroSettings || createDefaultPomodoroSettings();
        const specialties = Array.isArray(State.pomodoroSpecialties) ? State.pomodoroSpecialties : [];
        return [
            State.globalStats?.pomodoros || 0,
            todayPomodoros,
            streak,
            ...
            settings.dailyGoal,
            settings.autoStartNext ? 1 : 0,
            settings.systemNotifications ? 1 : 0,
            specialties.join(","),
            selectedSpecialtyLabels.join(","),
            weeklySeries.map(item => `${item.label}:${item.count}`).join("|"),
            recentLog.map(entry => [
                entry.id || "",
                entry.mode || "",
                entry.completedAt || 0,
                entry.durationSeconds || 0,
                Array.isArray(entry.specialties) ? entry.specialties.join(",") : "",
                entry.focusLabel || ""
            ].join("~")).join("|")
        ].join("::");
    };
    const renderPomodoroHub = () => {
        ...
        const isPomodoroViewActive = State.view === "view-pomodoro";
        ...
        if (isPomodoroViewActive) {
            const staticRenderKey = getPomodoroStaticRenderKey(
                weeklySeries,
                recentLog,
                selectedSpecialtyLabels,
                todayPomodoros,
                streak
            );
            if (staticRenderKey !== lastPomodoroStaticRenderKey) {
                lastPomodoroStaticRenderKey = staticRenderKey;
    ...
    const populateAccountProfileOptions = () => {
        populateSelectOptions($("auth-specialty"), ENARM_SPECIALTY_OPTIONS, "Elige tu especialidad objetivo");
        populateSelectOptions($("profile-specialty"), ENARM_SPECIALTY_OPTIONS, "Aun sin decidir");
        populateSelectOptions($("auth-target-year"), ENARM_TARGET_YEAR_OPTIONS, "Elige tu año objetivo");
        populateSelectOptions($("profile-target-year"), ENARM_TARGET_YEAR_OPTIONS, "Sin definir");
    };
    ...
    document.addEventListener("DOMContentLoaded", () => {
        ...
        if (btnSaveProfile) {
            btnSaveProfile.addEventListener("click", () => {
                ...
                if (State.userPhone && !isValidContactPhone(State.userPhone)) {
                    showNotification("El telefono debe tener entre 10 y 15 digitos.", "error");
                    return;
                }
                State.userTargetYear = $("profile-target-year")?.value || "";
        ...
        if (btnPerformanceSummary) {
            btnPerformanceSummary.addEventListener("click", () => $("nav-historial").click());
        }
        const copyReferralCode = () => {
            const code = State.referralCode || ($("profile-referral-code") ? $("profile-referral-code").value : "");
            if (!code) return showNotification("Tu codigo se esta generando. Intenta de nuevo en unos segundos.", "info");
            navigator.clipboard.writeText(code).then(() => {
                showNotification("Codigo de referido copiado.", "success");
            }).catch(() => {
                showNotification("No se pudo copiar el codigo.", "error");
            });
        };

        const btnCoinsCard = $("card-coins");
        const btnReferralsBack = $("btn-referrals-back");
        const btnCopyReferralHero = $("btn-copy-referral-hero");
        const btnCopyReferralPanel = $("btn-copy-referral-panel");
        const btnReferralsOpenPremium = $("btn-referrals-open-premium");
        if (btnCoinsCard) btnCoinsCard.addEventListener("click", () => showView("view-referrals"));
        if (btnReferralsBack) btnReferralsBack.addEventListener("click", () => $("nav-dashboard").click());
        if (btnCopyReferralHero) btnCopyReferralHero.addEventListener("click", copyReferralCode);
        if (btnCopyReferralPanel) btnCopyReferralPanel.addEventListener("click", copyReferralCode);
        if (btnReferralsOpenPremium) btnReferralsOpenPremium.addEventListener("click", () => openRedeemModal("Comparte tu codigo de referido y ayuda a tus amigos a desbloquear Premium."));
        const withdrawalForm = $("withdrawal-request-form");
        if (withdrawalForm) {
        ...
        const btnCopyReferral = $("btn-copy-referral-code");
        if (btnCopyReferral) {
            btnCopyReferral.addEventListener("click", () => {
                const referralInput = $("profile-referral-code");
                const referralVal = referralInput ? referralInput.value : State.referralCode;
                if (referralVal) {
                    navigator.clipboard.writeText(referralVal).then(() => {
                        showNotification("Codigo de referido copiado.", "success");
                    }).catch(() => {
                        showNotification("No se pudo copiar el codigo.", "error");
                    });
        ...
        const flashcardModal = $("flashcard-image-modal");
        const btnCloseFlashcardModal = $("btn-close-flashcard-modal");
        if (btnCloseFlashcardModal) {
            btnCloseFlashcardModal.addEventListener("click", closeFlashcardImageModal);
        }
        if (flashcardModal) {
            flashcardModal.addEventListener("click", (event) => {
                if (event.target === flashcardModal) closeFlashcardImageModal();
            });
        }
        document.addEventListener("keydown", (event) => {
            if (event.key === "Escape" && flashcardModal && flashcardModal.style.display === "flex") {
                closeFlashcardImageModal();
            }
        });
        ...
        function clearAccountLocalData() {
            ...
            localStorage.removeItem("enarm_target_year");
        ...
        const initCloudFeatures = () => {
            ...
            const renderCommunityName = (name, isPremium, extraMarkup = "") => `
                <span class="community-name-text">${escapeHtml(name || "Aspirante")}</span>
                ${renderCommunityPremiumBadge(isPremium)}
                ${extraMarkup}
            `;
        ...
        const setupChallengeLogic = () => {
                    ...
                    await withTemporaryButtonLabel(btnSend, "Preparando...", async () => {
                        ...
                        selectedCbs.forEach(cb => {
                            participants[cb.value] = {
                                ...
                                isPremium: cb.getAttribute("data-premium") === "1",
                                score: null,
                                status: "pending",
                                timestamp: null
                            };
            ...
            const renderChallenges = () => {
                ...
                const renderCard = (ch) => {
                    ...
                    const others = Object.entries(ch.participants || {})
                        .filter(([participantUid]) => participantUid !== uid)
                        .map(([participantUid, participant]) => ({
                            ...participant,
                            uid: participantUid,
                            isPremium: getCommunityPremiumState(participantUid, participant?.isPremium === true)
                        }));
                    const opponentMarkup = others.length === 1
                        ? renderCommunityName(others[0].name, others[0].isPremium)
            ...
            window.showChallengeRanking = async (id) => {
                ...
                const pts = Object.entries(ch.participants || {})
                    .map(([participantUid, participant]) => ({
                        ...participant,
                        uid: participantUid,
                        isPremium: getCommunityPremiumState(participantUid, participant?.isPremium === true)
                ...
                list.innerHTML = pts.map((p, idx) => `
                    <div style="display:flex; justify-content:space-between; align-items:center; padding: 10px; background: rgba(255,255,255,0.03); border-radius: 10px;">
                        <div style="display:flex; align-items:center; gap:10px;">
                            ...
                            <span class="community-name-row" style="font-size: 14px;">${renderCommunityName(p.name, p.isPremium)}</span>
                        </div>
        ...
        const bindEntitlementListener = (uid) => {
            ...
            State.entitlementUnsub = window.FB.onSnapshot(ref, (snap) => {
                ...
                if (State.entitlement.status === "active" && State.entitlement.source === "manual_transfer" && State.entitlement.manualPaymentRequestId) {
                    const seenRequestId = localStorage.getItem(MANUAL_ENTITLEMENT_SEEN_KEY) || "";
                    if (seenRequestId !== State.entitlement.manualPaymentRequestId) {
                        localStorage.setItem(MANUAL_ENTITLEMENT_SEEN_KEY, State.entitlement.manualPaymentRequestId);
                        setTimeout(() => {
                            showNotification("Tu transferencia fue validada. Premium ya está activo.", "success");
        ...
        const bindGlobalPremiumListener = () => {
            if (!window.FB || !window.FB.db || !window.FB.onSnapshot) {
                State.globalPremiumLoaded = true;
                return;
            }
        ...
        const redeemCode = async (opts = {}) => {
            ...
            try {
                ...
                if (msg.includes("invalid_referral")) {
                    showNotification("Codigo de referido invalido. Revisalo o deja ese campo vacio.", "warning");
                    ...
                    showNotification("Codigo invalido o no autorizado.", "error");
                } else if (msg.includes("permission-denied") || msg.includes("Missing or insufficient permissions")) {
        ...
        const setupFirebaseAuthAndUI = () => {
            ...
            if (authOverlay && loginForm) {
                ...
                const setPlanButtonsBusy = (busy, activePlan = "") => {
                    Object.keys(TRANSFER_PLANS).forEach(planId => {
                        document.querySelectorAll(`[data-plan-id="${planId}"]`).forEach(btn => {
                            ...
                            btn.innerHTML = busy && planId === activePlan
                ...
                const ensureTransferProfile = async () => {
                    const currentUser = window.FB && window.FB.auth ? window.FB.auth.currentUser : null;
                    if (!currentUser || !window.FB.db || !window.FB.doc || !window.FB.runTransaction) return null;
                    if (State.transferProfile && /^\d{10}$/.test(State.transferProfile.transferReference || "")) {
                        return State.transferProfile;
                    }
                    if (State.transferProfilePromise) return State.transferProfilePromise;

                    const makeTransferReference = () => {
                        if (window.crypto && typeof window.crypto.getRandomValues === "function" && typeof BigInt === "function") {
                            const values = new Uint32Array(2);
                            window.crypto.getRandomValues(values);
                            const random = ((BigInt(values[0]) << 32n) | BigInt(values[1])) % 9000000000n;
                            return String(1000000000n + random);
                        }
                        return String(1000000000 + Math.floor(Math.random() * 9000000000));
                    };

                    State.transferProfilePromise = window.FB.runTransaction(window.FB.db, async (tx) => {
                        const profileRef = window.FB.doc(window.FB.db, "payment_profiles", currentUser.uid);
                        const profileSnap = await tx.get(profileRef);
                        if (profileSnap.exists()) {
                            const existing = profileSnap.data() || {};
                            if (/^\d{10}$/.test(String(existing.transferReference || ""))) {
                                return { transferReference: String(existing.transferReference) };
                            }
                        }

                        for (let attempt = 0; attempt < 16; attempt += 1) {
                            const transferReference = makeTransferReference();
                            const referenceRef = window.FB.doc(window.FB.db, "transfer_references", transferReference);
                            const referenceSnap = await tx.get(referenceRef);
                            if (referenceSnap.exists()) continue;

                            const now = new Date();
                            tx.set(referenceRef, { uid: currentUser.uid, transferReference, createdAt: now });
                            tx.set(profileRef, {
                                uid: currentUser.uid,
                        ...
                        throw new Error("transfer_reference_exhausted");
                    }).then((profile) => {
                        if (!profile || !/^\d{10}$/.test(profile.transferReference || "")) {
                            throw new Error("transfer_reference_invalid");
                        }
                        State.transferProfile = profile;
                        return profile;
                    }).finally(() => {
                        State.transferProfilePromise = null;
                    });
                    return State.transferProfilePromise;
                };
                ...
                const beginPlanTransfer = async (planId, options = {}) => {
                    ...
                    if (!plan || checkoutStarting) return;
                    if ($("redeem-modal")?.style.display === "flex") closeRedeemModal();
                    localStorage.setItem(PENDING_PLAN_STORAGE_KEY, planId);

                    const currentUser = window.FB && window.FB.auth ? window.FB.auth.currentUser : null;
                    if (!currentUser) {
                        trackEvent("select_plan", { plan_id: planId, authenticated: false });
                        showAuthFromLanding({ register: true });
                        showNotification("Crea tu cuenta o inicia sesión para ver los datos de transferencia.", "info");
                        return;
                    }

                    if (!window.FB || !window.FB.db || !window.FB.doc || !window.FB.runTransaction) {
                        showNotification("Firebase todavía no está disponible. Intenta de nuevo.", "warning");
                        return;
                    }

                    const currentExpiry = State.entitlement && State.entitlement.expiresAt
                        ? new Date(State.entitlement.expiresAt).getTime()
                        : 0;
                    if (State.entitlement && State.entitlement.status === "active" && currentExpiry >= new Date(plan.expiresAt).getTime()) {
                        localStorage.removeItem(PENDING_PLAN_STORAGE_KEY);
                        showNotification("Tu cuenta ya tiene este plan o uno con mayor vigencia.", "info");
                        return;
                    }

                    checkoutStarting = true;
                    setPlanButtonsBusy(true, planId);
                    if (!options.resume) trackEvent("select_plan", { plan_id: planId, authenticated: true });
                    trackEvent("begin_bank_transfer", { plan_id: planId });

                    try {
                        const profile = await ensureTransferProfile();
                        if (!profile) throw new Error("transfer_profile_unavailable");
                        State.selectedTransferPlan = planId;
                        const nameEl = $("transfer-plan-name");
                        const amountEl = $("transfer-plan-amount");
                        const referenceEl = $("transfer-reference");
                        ...
                        const paymentCheck = $("transfer-payment-confirm");
                        const legalCheck = $("transfer-legal-check");
                        if (paymentCheck) paymentCheck.checked = false;
                        if (legalCheck) legalCheck.checked = false;
                        const sentBtn = $("btn-transfer-sent");
                        if (sentBtn) {
                            sentBtn.disabled = false;
                            sentBtn.textContent = "Confirmar pago realizado";
                        }
                        ...
                        console.error("No se pudo preparar la transferencia:", err);
                        ...
                        if (code.includes("already-exists") || message.includes("already-exists")) {
                            ...
                            showNotification("No pudimos generar tu referencia. Intenta de nuevo.", "error");
                        }
                    } finally {
                ...
                const resumePendingTransfer = () => {
                    const pendingPlan = localStorage.getItem(PENDING_PLAN_STORAGE_KEY) || "";
                ...
                ["btn-close-transfer", "btn-transfer-later"].forEach(id => {
                    const btn = $(id);
                    if (btn) btn.addEventListener("click", closeTransferModal);
                });

                const transferModal = $("transfer-modal");
                if (transferModal) transferModal.addEventListener("click", (event) => {
                ...
                document.querySelectorAll("[data-copy-target]").forEach(btn => {
                    btn.addEventListener("click", async () => {
                        ...
                        if (!value || value === "Cargando...") return;
                        try {
                            await navigator.clipboard.writeText(value);
                            showNotification("Dato copiado.", "success");
                        } catch (err) {
                            showNotification(`Copia manualmente: ${value}`, "info");
                        }
                    });
                });

                const transferSentBtn = $("btn-transfer-sent");
                if (transferSentBtn) transferSentBtn.addEventListener("click", async () => {
                    const plan = getTransferPlan(State.selectedTransferPlan);
                    const currentUser = window.FB && window.FB.auth ? window.FB.auth.currentUser : null;
                    if (!plan || !currentUser || State.transferSubmitting) return;

                    const paymentConfirmed = $("transfer-payment-confirm") && $("transfer-payment-confirm").checked;
                    const legalAccepted = $("transfer-legal-check") && $("transfer-legal-check").checked;
                    if (!paymentConfirmed || !legalAccepted) {
                        showNotification("Confirma la transferencia y acepta los términos antes de continuar.", "warning");
                        return;
                    }

                    const referralCode = String($("transfer-referral-code")?.value || "").trim().toUpperCase();
                    if (referralCode && !/^EL[A-Z0-9]{6,12}$/.test(referralCode)) {
                        showNotification("El código de referido no tiene un formato válido.", "warning");
                        return;
                    }
                    ...
                    try {
                        const profile = await ensureTransferProfile();
                        if (!profile) throw new Error("transfer_profile_unavailable");
                        const requestId = currentUser.uid + "_" + plan.id;
                        const requestRef = window.FB.doc(window.FB.db, MANUAL_PAYMENT_REQUESTS_COLLECTION, requestId);
                        const existingSnap = await window.FB.getDoc(requestRef);
                        const existing = existingSnap.exists() ? (existingSnap.data() || {}) : {};
                        if (existing.status === "approved") throw new Error("payment_already_approved");
                        ...
                        await window.FB.setDoc(requestRef, requestData, { merge: true });
                        State.myManualPaymentRequests = [
                            ...State.myManualPaymentRequests.filter(item => item.id !== requestId),
                            { id: requestId, ...requestData }
                        ];
                        syncPaymentPendingBanner();
                        trackEvent("bank_transfer_notified", { plan_id: plan.id });
                        closeTransferModal();
                        showView("view-dashboard");
                        showNotification("Tu pago quedó pendiente de comprobación. Te avisaremos cuando Premium esté activo.", "info");
                    } catch (err) {
                        console.error("No se pudo enviar el aviso de transferencia:", err);
                        const message = String((err && (err.code || err.message)) || "");
                        showNotification(
                            message.includes("already") ? "Este pago ya fue aprobado." : "No se pudo enviar la confirmación. Intenta de nuevo.",
                            message.includes("already") ? "info" : "error"
                        );
                        transferSentBtn.disabled = false;
                        transferSentBtn.textContent = "Confirmar pago realizado";
                    } finally {
                        State.transferSubmitting = false;
                    }
                });
                ...
                document.querySelectorAll("[data-plan-id]").forEach(el => {
                    el.addEventListener("click", () => void beginPlanTransfer(el.dataset.planId || ""));
                });

                const demoCard = $("landing-demo-card");
                const demoFeedback = $("landing-demo-feedback");
                const demoFeedbackTitle = $("landing-demo-feedback-title");
                if (demoCard && demoFeedback && demoFeedbackTitle) {
                    const demoOptions = Array.from(demoCard.querySelectorAll(".landing-demo-option"));
                    demoOptions.forEach(option => {
                        option.addEventListener("click", () => {
                            if (demoCard.classList.contains("is-answered")) return;
                            const correct = option.dataset.demoAnswer === "correct";
                            demoOptions.forEach(item => {
                                item.disabled = true;
                                if (item.dataset.demoAnswer === "correct") item.classList.add("is-correct");
                            });
                            if (!correct) option.classList.add("is-incorrect");
                            demoFeedbackTitle.textContent = correct ? "Correcto: reperfusión inmediata" : "Revisa la prioridad: reperfusión inmediata";
                            demoFeedback.hidden = false;
                            demoCard.classList.add("is-answered");
                            trackEvent("landing_demo_answer", { correct });
                        });
                    });
                }

                // Hamburger menu toggle
                const hamburger = $("landing-hamburger");
                ...
                if (landingPage) {
                    landingPage.querySelectorAll('a[href^="#landing-"]').forEach(link => {
                ...
                if (State.guestExamActive) {
                    ...
                    const appLayout = document.querySelector(".app-layout");
                    if (appLayout) appLayout.style.display = "none";
                    trackEvent("landing_view", { page_variant: "pricing_transfer_v1" });
                } else {
                    if (landingPage) landingPage.classList.add("hidden");
                    authOverlay.classList.remove("active");
                }
                    ...
                    window.FB.onAuthStateChanged(window.FB.auth, async (user) => {
                        ...
                        if (user) {
                            ...
                            if (State.adminUsersUnsub) {
                                State.adminUsersUnsub();
                                State.adminUsersUnsub = null;
                            }
                            if (State.adminDirectoryUnsub) {
                                State.adminDirectoryUnsub();
                                State.adminDirectoryUnsub = null;
                            }
                ...
                const authUsername = $("auth-username");
                const authSpecialty = $("auth-specialty");
                ...
                const authRegisterLink = document.querySelector(".auth-footer a");
                if (authRegisterLink) {
