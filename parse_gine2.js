...
function parseTextFile(filename, defaultSpecialty) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (line.match(/^Caso cl[ií]nico/i) || line.match(/^(Una |Un |Mujer |Masculino |Paciente )/) && mode == null) {
            // Some PDFs might not have "Caso clínico:" explicitly, they just start the text.
            // Let's handle the explicit one:
            if (line.match(/^Caso cl[ií]nico/i)) {
                mode = 'case_text';
                continue;
            } else if (line.match(/^(Una |Un |Mujer |Masculino |Paciente |Escolar )/) && mode == null) {
                mode = 'case_text';
        ...
        if (line.match(/^Pregunta \d+/) || line.match(/^Pregunta$/) || line.match(/^Pregunta\./)) {
            let qText = line.replace(/^Pregunta \d*\.?\s*:?\s*/, '').trim();
            currentQuestion = {
        ...
        if (mode === 'case_text') {
            ...
            if (m) {
                ...
                optText = optText.substring(m[0].length).trim(); // remove A) or *A)
    ...
    cases.forEach(c => {
        ...
        if (diffLower.includes('baja') || diffLower.includes('fácil') || diffLower.includes('facil')) normalizedDiff = 'baja';
...
let gine2 = parseTextFile('gine2_text.txt', 'gine');

fs.writeFileSync('parsed_gine2.json', JSON.stringify(gine2, null, 2));
console.log(`Parsed ${gine2.length} new gine cases from gine2_text.txt`);

// Merge with questions.js directly
let oldQ = require('./questions.js');
let combined = oldQ.concat(gine2);
...
console.log(`Successfully combined ${oldQ.length} old questions with ${gine2.length} new cases. New total: ${combined.length}`);
