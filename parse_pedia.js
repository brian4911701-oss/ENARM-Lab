...
let pedia = parseTextFile('pedia_text.txt', 'ped');

fs.writeFileSync('parsed_pedia.json', JSON.stringify(pedia, null, 2));
console.log(`Parsed ${pedia.length} new pedia cases from pedia_text.txt`);

// Merge with questions.js directly
let oldQ = require('./questions.js');
let combined = oldQ.concat(pedia);
...
console.log(`Successfully combined ${oldQ.length} old questions with ${pedia.length} new cases. New total: ${combined.length}`);
