...
let ciru2 = parseTextFile('ciru2_text.txt', 'cir');

fs.writeFileSync('parsed_ciru2.json', JSON.stringify(ciru2, null, 2));
console.log(`Parsed ${ciru2.length} new ciru cases from ciru2_text.txt`);

// Merge with questions.js directly
let oldQ = require('./questions.js');
let combined = oldQ.concat(ciru2);
...
console.log(`Successfully combined ${oldQ.length} old questions with ${ciru2.length} new cases. New total: ${combined.length}`);
