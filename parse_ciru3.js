...
let ciru3 = parseTextFile('ciru3_text.txt', 'cir');

fs.writeFileSync('parsed_ciru3.json', JSON.stringify(ciru3, null, 2));
console.log(`Parsed ${ciru3.length} new ciru cases from ciru3_text.txt`);

// Merge with questions.js directly
let oldQ = require('./questions.js');
let combined = oldQ.concat(ciru3);
...
console.log(`Successfully combined ${oldQ.length} old questions with ${ciru3.length} new cases. New total: ${combined.length}`);
